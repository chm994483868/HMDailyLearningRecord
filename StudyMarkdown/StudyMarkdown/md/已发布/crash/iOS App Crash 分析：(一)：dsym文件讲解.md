...
&emsp;当异常处理程序堆栈中没有异常处理程序时，除非在发布通知期间引发异常，否则此方法调用未捕获异常处理程序（`void uncaughtExceptionHandler(NSException *exception)`），在该处理程序中可以执行最后一分钟的日志记录（实测可以执行好久，超过 1 分钟）。无论 `uncaughtExceptionHandler` 执行了什么操作，程序都会终止。
