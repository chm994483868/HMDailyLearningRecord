...
&emsp;TypeScript 使用 Map 类型和 new 关键字来创建 Map：

```typescript
let myMap = new Map();
```

&emsp;初始化 Map，可以以数组的格式来传入键值对：
...
nameSiteMapping.set("Apple", 2);
nameSiteMapping.set("Taobao", 3);

// 获取键对应的值
console.log(nameSiteMapping.get("Apple"));
// 输出 
2

// 判断 Map 中是否包含键对应的值
console.log(nameSiteMapping.has("Apple"));
// 输出
true
...
// 删除 Apple
console.log(nameSiteMapping.delete("Apple"));
// 输出
true
...
&emsp;如果我们仅使用 tsc 编译的话，会报如下错误：

```typescript
First_TypeScript.ts:206:27 - error TS2583: Cannot find name 'Map'. Do you need to change your target library? Try changing the 'lib' compiler option to 'es2015' or later.

206 let nameSiteMapping = new Map();
                              ~~~


Found 1 error in First_TypeScript.ts:206

```

&emsp;我们需要使用 `tsc --target es6` 进行编译。

### 迭代 Map

&emsp;Map 对象中的元素是按顺序插入的，我们可以迭代 Map 对象，每一次迭代返回 `[key, value]` 数组。TypeScript使用 for...of 来实现迭代：
